...
class User extends Api
{
    ...
    {
        $param = $this->_createParameters('mobile,password');
        if (!preg_match('/^1[34578][0-9]{9}$/', $param['mobile'])) {
            $this->_returnError('10008', '手机号码不合法');
        }

        if (!preg_match('/^[\w+]{6,16}$/', $param['password'])) {
            $this->_returnError('10004', '密码不合法');
        }

        $model_user = new \app\index\model\User;
        $user_info = $model_user->toFind(['mobile' => $param['mobile']], 'user_id,password,pwd_key');
        if (!$user_info) {
            $this->_returnError('10009', '用户名或密码错误');
        }

        $logic_user = new \app\index\logic\User;
        if ($logic_user->passwordEncryption($param['password'], $user_info['pwd_key']) != $user_info['password']) {
            $this->_returnError('10009', '用户名或密码错误');
        }
    ...
    {
        $param = $this->_createParameters('mobile,password,note_code,platfrom');
        if (!preg_match('/^1[34578][0-9]{9}$/', $param['mobile'])) {
            $this->_returnError(10008, '手机号码不合法');
        }
        ...
        if (!in_array($param['platfrom'], $all_from)) {
            $this->_returnError(10006, '平台不合法');
        }

        $logic_user = new \app\index\logic\User;
        $code_info = $logic_user->getNoteCode($param['mobile']);
        if (!$code_info || $note_code != $code_info['code']) {
            $this->_returnError(10011, '短信验证码错误');
        }

        if ($param['mobile'] != $code_info['mobile']) {
            $this->_returnError(10012, '接收短信的手机号与提交的手机号不匹配');
        }

        $model_user = new \app\index\model\User;

        $user_info = $model_user->toFind(['mobile' => $param['mobile']]);
        if ($user_info) {
        ...
        $data['password'] = $logic_user->passwordEncryption($param['password'], $data['pwd_key']);
        $data['nick_name'] = $param['mobile'];
        $data['user_from'] = $param['platfrom'];
        ...
        //删除保存的短信验证码信息
        $logic_user->delNoteCode($param['mobile']);
        $data = [
            'passport' => $passport
        ];
        $this->_returnData($data);
    ...
    {
        $param = $this->_createParameters('mobile');
        if (!preg_match('/^1[34578][0-9]{9}$/', $param['mobile'])) {
            $this->_returnError('10008', '手机号码不合法');
        }
        $user = new \app\index\logic\User;
        if ($user->getNoteCode($param['mobile'])) {
        ...
        if (isset($resp['result']['err_code']) && $resp['result']['err_code'] == 0) {
            $user->saveNoteCode(array('code' => $code, 'mobile' => $param['mobile']));
            $this->_returnData();
